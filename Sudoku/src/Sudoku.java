...
public class Sudoku {
	...
	private boolean solve(int r, int c) {
		if (r == 8 && c == 8) { // If we reach the lower right corner of the board the sudoku is solved.
			return true;
		} else {
			for (int i = 1; i <= 9; i++) {		// Loops through values 1-9 to insert in to the sudoku
				...
				} else if (grid[r][c] == 0 && ruleCheck(r, c, i)) {		// Makes sure the value is valid for the specified field
					if (c < 8) {				
						grid[r][c] = i;			// If the value is valid and we are inside the field, add value i.
						return solve(r, c + 1);		// Increase column value until we reach 8, at which point we reset to one and increase row.
					} else if (r < 8) {
